    ...
    <div className="settings-layout">
      <section className="panel">
        ...
        <div className="settings-form">
          ...
          <label><span>单用户图片并发</span><input type="number" min={1} value={Number(settings.image_user_concurrency || 4)} onChange={(event) => updateField("image_user_concurrency", Number(event.target.value))} /></label>
...
function RegisterPanel({ token, registerRuntime, setRegisterRuntime, toast }: { token: string; registerRuntime: RegisterRuntime | null; setRegisterRuntime: React.Dispatch<React.SetStateAction<RegisterRuntime | null>>; toast: (type: Toast["type"], message: string) => void }) {
  ...
  const registerProvider = String(registerMail.provider || "inbucket");
  ...
  function registerDraftFromRuntime(runtime: RegisterRuntime | null) {
    ...
    return {
      ...
      provider: String(mail.provider || "inbucket"),
      ...
      spamokBaseURL: String(mail.spamok_base_url || "https://spamok.com")
    };
  }
  ...
  }, [draftDirty, registerRuntime, registerConfig?.proxy, registerConfig?.mode, registerConfig?.total, registerConfig?.threads, registerConfig?.target_quota, registerConfig?.target_available, registerConfig?.check_interval, registerProvider, registerMail.inbucket_api_base, registerDomainsText, registerMail.random_subdomain, registerMail.spamok_base_url]);
